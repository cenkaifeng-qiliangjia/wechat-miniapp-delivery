...
| `weapp_test_automation` | Deterministic unit or E2E proof is needed | `miniprogram-simulate` plus repo runner, then `miniprogram-automator` or `minium` | Node for JS runners, Python `>=3.8` for `minium` | Unit-only or simulate-only | coverage or flow evidence, residual risk |
...
Use for unit, simulate, or E2E evidence.
